...
class SiteController extends Controller
{
    ...
    protected function userData()
    {
        ...
        if ($request->age) {
            ...
            $minAge = trim($requestedAge[0]);
            $maxAge = trim(rtrim($requestedAge[1], 'Years'));


            // Calculate the minimum and maximum birth dates
            $minBirthDate = (new DateTime())->sub(new DateInterval('P' . $maxAge . 'Y'))->format('Y-m-d');
            $maxBirthDate = (new DateTime())->sub(new DateInterval('P' . $minAge . 'Y'))->format('Y-m-d');


            if ($min != $minAge || $max != $maxHeight) {


                $query = $query->whereHas('BasicInfo', function ($q) use ($minBirthDate, $maxBirthDate) {
